<route lang="jsonc" type="page">
{
  ...
  "style": {
    ...
    "navigationBarTitleText": "Đăng nhập"
  }
}
</route>
...
<template>
  <view class="app-container box-border h-screen w-full" :class="{ 'dark-mode': themeStore.isDark }" :style="{ paddingTop: `${safeAreaInsets?.top}px` }">
	...
	<view class="top-right-buttons" :style="{ top: `${safeAreaInsets?.top + 10}px` }">
	  <view class="theme-btn" @click="themeStore.toggleTheme()">
      <text class="theme-text-icon">{{ themeStore.isDark ? '☀' : '🌙' }}</text>
	  </view>
...
.app-container.dark-mode {
  ...
  .subtitle { color: rgba(226, 232, 240, 0.8) !important; }
  .input-wrapper,
  .mobile-input-wrapper,
  .area-code-selector { background: #111827 !important; border-color: #334155 !important; }
  :deep(.styled-input) { color: #f3f4f6 !important; }
  :deep(.styled-input)::placeholder { color: #94a3b8 !important; }
  .area-code-text, .register-link, .forgot-text { color: #c7d2fe !important; }
  .server-btn, .theme-btn { background: rgba(15, 23, 42, 0.45); }
}
</style>
