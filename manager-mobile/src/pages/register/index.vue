<route lang="jsonc" type="page">
{
  ...
  "style": {
    ...
    "navigationBarTitleText": "Đăng ký"
  }
}
</route>
...
const selectedAreaCode = ref('+84')
const selectedAreaName = ref('Việt Nam')
...
const areaCodeList = computed(() => {
  return configStore.config.mobileAreaList || [{ name: 'Việt Nam', key: '+84' }]
})
...
<template>
  <view class="app-container box-border h-screen w-full" :class="{ 'dark-mode': themeStore.isDark }" :style="{ paddingTop: `${safeAreaInsets?.top}px` }">
    <view class="header">
...
.app-container.dark-mode {
  background: linear-gradient(145deg, #0f172a, #1e3a8a, #1f2937, #0f172a);
  .form-container .form { background: rgba(17, 24, 39, 0.96); border-color: rgba(148, 163, 184, 0.15); box-shadow: 0 20rpx 60rpx rgba(0,0,0,.35); }
  .welcome-text { color: #f8fafc !important; }
  .subtitle { color: rgba(226,232,240,.8) !important; }
  .input-wrapper, .mobile-input-wrapper, .area-code-selector, .sms-btn, .back-button { background: #111827 !important; border-color: #334155 !important; }
  :deep(.styled-input) { color: #f3f4f6 !important; }
  :deep(.styled-input)::placeholder { color: #94a3b8 !important; }
  .area-code-text, .hint-text, .login-link { color: #cbd5e1 !important; }
}
</style>
