...
<template>
  <view class="device-container" style="min-height: 100%;">
    <!-- 加载状态 -->
    ...
    <view v-else-if="!loading" class="empty-container">
      <view class="flex flex-col items-center justify-center p-[100rpx_40rpx] text-center">
        ...
        <text class="mb-[16rpx] text-[32rpx] empty-primary font-medium">
          {{ t('device.noDevice') }}
        </text>
        <text class="text-[26rpx] empty-secondary leading-[1.5]">
...
.device-card { background: var(--card-bg); border: 1rpx solid var(--border-color); padding: 32rpx; }
.device-title { color: var(--text-primary); }
.device-meta { color: var(--text-secondary); }
.device-toggle-shell { border: 1rpx solid var(--border-color); border-radius: 12rpx; background: var(--surface-bg); padding: 16rpx 20rpx; }
.device-toggle-title { color: var(--text-primary); }
.empty-primary { color: var(--text-primary); }
.empty-secondary { color: var(--text-secondary); }
