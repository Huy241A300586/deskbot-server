<route lang="jsonc" type="page">
{
  ...
  "style": {
    ...
    "navigationBarTitleText": "Quên mật khẩu"
  }
}
</route>
...
const areaCodeList = computed(() =>
  (configStore.config.mobileAreaList?.length ? configStore.config.mobileAreaList : [{ name: "Việt Nam", key: "+84" }]).map((item) => {
    return {
      value: item.key,
...
.app-container.dark-mode {
  ...
  .input-wrapper, .mobile-input-wrapper, .area-code-selector, .sms-button, .back-button { background: #111827 !important; border-color: #334155 !important; }
  :deep(.styled-input) { color: #f3f4f6 !important; }
  :deep(.styled-input)::placeholder { color: #94a3b8 !important; }
  .area-code-text, .hint-text { color: #cbd5e1 !important; }
}
</style>
