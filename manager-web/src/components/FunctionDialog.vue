<template>
  <el-drawer :visible.sync="dialogVisible" direction="rtl" size="80%" :wrapperClosable="false" :withHeader="false">
    ...
    <div class="mcp-access-point" v-if="featureStatus.mcpAccessPoint">
      ...
      <div class="mcp-market-mini">
        <div class="mcp-header">
          <h3 class="bold-title">{{ $t('functionDialog.mcpMarketMini') }}</h3>
          <p class="market-subtitle">{{ $t('functionDialog.mcpMarketMiniDesc') }}</p>
        </div>

        <div class="market-grid">
          <div v-for="preset in mcpMarketPresets" :key="preset.id" class="market-card">
            <div class="market-card-top">
              <div>
                <div class="preset-title">{{ preset.name }}</div>
                <div class="preset-category">{{ preset.category }}</div>
              </div>
              <span class="preset-badge">{{ preset.transport }}</span>
            </div>

            <p class="preset-description">{{ preset.description }}</p>

            <div class="preset-section">
              <div class="preset-section-title">{{ $t('functionDialog.recommendedTools') }}</div>
              <div class="tools-grid compact">
                <el-button v-for="tool in preset.tools" :key="preset.id + '-' + tool" size="mini" class="tool-btn" plain>
                  {{ tool }}
                </el-button>
              </div>
            </div>
            ...
            <div class="preset-section">
              <div class="preset-section-title">{{ $t('functionDialog.basicConfig') }}</div>
              <div class="config-chips">
                <span v-for="item in preset.basicConfig" :key="preset.id + '-cfg-' + item" class="config-chip">{{ item }}</span>
              </div>
            </div>

            <div class="preset-section">
              <div class="preset-section-title">{{ $t('functionDialog.importSnippet') }}</div>
              <pre class="snippet-box">{{ formatPresetSnippet(preset) }}</pre>
            </div>

            <div class="preset-actions">
              <el-button size="mini" type="primary" @click="copyPresetSnippet(preset)">
                {{ $t('functionDialog.copyImportSnippet') }}
              </el-button>
              <el-button size="mini" plain @click="fillPresetPreview(preset)">
                {{ $t('functionDialog.previewPresetTools') }}
              </el-button>
            </div>
          </div>
        </div>
      ...
      mcpMarketPresets: [
        {
          ...
          transport: 'stdio',
          description: 'Preset mẫu để DeskBot duyệt thư mục, đọc tài liệu và mở rộng khả năng xử lý file nội bộ.',
          tools: ['list_files', 'read_file', 'search_files'],
          basicConfig: ['command: npx', 'args: @modelcontextprotocol/server-filesystem', 'scope: thư mục được phép truy cập'],
          snippet: {
            deskbotFiles: {
              command: 'npx',
              args: ['-y', '@modelcontextprotocol/server-filesystem', '/data/deskbot'],
              env: {}
            }
          }
        },
        {
          id: 'deskbot-fetch',
          name: 'DeskBot Web Fetch',
          category: 'Web & tra cứu',
          transport: 'stdio',
          description: 'Preset mẫu để DeskBot lấy nội dung URL, hỗ trợ đọc tài liệu web hoặc knowledge page nhanh.',
          tools: ['fetch', 'robots', 'head'],
          basicConfig: ['command: uvx', 'package: mcp-server-fetch', 'allow outbound network'],
          snippet: {
            deskbotFetch: {
              command: 'uvx',
        ...
        {
          id: 'deskbot-postgres',
          name: 'DeskBot Postgres',
          category: 'Dữ liệu',
          transport: 'stdio',
          description: 'Preset mẫu để DeskBot truy vấn PostgreSQL ở phạm vi kiểm soát, phù hợp dashboard và dữ liệu nghiệp vụ.',
          tools: ['query', 'describe_table', 'list_tables'],
          basicConfig: ['command: npx', 'args: @modelcontextprotocol/server-postgres', 'env: DATABASE_URL'],
          snippet: {
            deskbotPostgres: {
              command: 'npx',
  ...
  methods: {
    ...
    copyText(value, successMessage) {
      ...
      try {
        ...
        if (successful) {
          this.$message.success(successMessage || this.$t('functionDialog.copiedToClipboard'));
        } else {
    ...
    copyUrl() {
      this.copyText(this.mcpUrl, this.$t('functionDialog.copiedToClipboard'));
    },

    formatPresetSnippet(preset) {
      return JSON.stringify({ mcpServers: preset.snippet }, null, 2);
    },

    copyPresetSnippet(preset) {
      this.copyText(this.formatPresetSnippet(preset), this.$t('functionDialog.importSnippetCopied'));
    },

    fillPresetPreview(preset) {
      this.mcpTools = [...preset.tools];
      this.mcpStatus = 'connected';
      this.$message.success(this.$t('functionDialog.previewApplied'));
    },
